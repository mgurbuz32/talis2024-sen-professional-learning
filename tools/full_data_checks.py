...
def find_csv():
    with zipfile.ZipFile(ZIP) as z:
        ...
        if exact:
            ...
            cand = [n for n in names if 'TTGINTT4' in n.upper()]
            if not cand:
                raise RuntimeError('Could not locate ttgintt4.csv. CSV files: ' + str(names[:50]))
            target = cand[0]
        print('Using', target, flush=True)
        z.extract(target, '.')
    return Path(target)
...
def detect_sep(path):
    ...
    print('Detected delimiter:', repr(sep), flush=True)
    return sep
...
def system_brr(d, extra=None):
    ...
    reps = np.array([wls_beta(X, y, d[f'TRWGT{r}'])[idx] for r in range(1, 101)])
...
csv_path = find_csv(); sep = detect_sep(csv_path)
required = ['CNTRY','IDPOP','IDTQUEST','T4SESEN','TT4G21K','TT4G24K','T4TYEXPTT','T4TCSIZE','TT4G47E','TCHWGT','T4SELF','TT4G01','T4THEDAT','ADJRT24']
...
missing_core = [c for c in core_required if c not in head.columns]
if missing_core:
